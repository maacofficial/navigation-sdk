...
This package now supports React Native's New Architecture (Fabric + TurboModules). The library automatically detects which architecture is being used and adapts accordingly.

To enable New Architecture, set in your `android/gradle.properties` file:

```groovy
newArchEnabled=true
```

For detailed setup instructions and migration notes, see [NEW_ARCHITECTURE.md](./NEW_ARCHITECTURE.md).

> **Note**: If you're not ready to use New Architecture, you can continue using the legacy architecture by setting `newArchEnabled=false` (default).
...
This package now supports React Native's New Architecture (Fabric + TurboModules) on iOS as well. 

To enable New Architecture, set in your `ios/Podfile`:
...
> **Note**: If you're not ready to use New Architecture, you can continue using the legacy architecture by setting `ENV['RCT_NEW_ARCH_ENABLED'] = '0'` (recommended for most users until New Architecture becomes stable).
